    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            var tables = new ListsVM
            {
                ...
                Positions = _context.Positions.Where(n => n.Inactive == false).OrderBy(n => n.PositionName).ToList(),
            };
        ...
        ///Lunch Page Override and Main page lunch submission///
        public async Task<IActionResult> CreateLunchPage([Bind("Id,TimeEntered,TimeCleared,EmpSent,LunchTime,EmployeeId,ReasonId,EmpPosition,RlfPosition,LongerLunch,DblLunch")] Lunch lunch)
        {
            if (ModelState.IsValid)
            {
                lunch.TimeEntered = DateTime.Now;
                _context.Add(lunch);
                await _context.SaveChangesAsync();
                return RedirectToAction(nameof(Lunch));
            }
            ViewData["EmpPosition"] = new SelectList(_context.Positions, "Id", "Id", lunch.EmpPosition);
            ViewData["EmployeeId"] = new SelectList(_context.Employees, "Id", "DisplayName", lunch.EmployeeId);
            ViewData["ReasonId"] = new SelectList(_context.Reasons, "Id", "ReasonName", lunch.ReasonId);
